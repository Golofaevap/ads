function main() {
    ...
    const inputParameters = {
        url__: "https://google.com",
        trackingUrl__: "https://google.com",
        countriesStr: "austria;germany", // australia;austria;belgium;canada;denmark;finland;france;germany;ireland;italy;luxembourg;netherlands;new zealand;norway;spain;sweden;switzerland;united kingdom;united states
        samaraId: "123", // Любая строка или число
    };
    ...
    var _bgtLevel = (_bgt * 100 + Math.random() * 3100) * budgetMultiplier;
    if (accelerator) {
        _bgtLevel = (500000 * Math.random() + 50000) * budgetMultiplier;
    }
    var opts = getOpts(inputParameters, targetedCountries, _bgtLevel, adjustText, budgetMultiplier);
...
// *******************************************************************************************
// *******************************************************************************************
function getKeywords(bid) {
    const kws = ["keto preis", "keto kaufen", "keto amazon", "keto bestellen"];
    const keys = [];
    for (var i = 0; i < kws.length; i++) {
        keys.push({ word: kws[i], cpc: bid });
        keys.push({ word: '"' + kws[i] + '"', cpc: bid });
        keys.push({ word: "[" + kws[i] + "]", cpc: bid });
    }
    /**
...
function getSiteLinks() {
    return [
        {
            text: "Aktiven Fettverbrennung",
            desc1: "Beginnen Sie sofort nach der Einnahme",
            desc2: "Mit der aktiven Fettverbrennung",
            anchor: "#akt",
        },
        { text: "Bekämpft das Fett", desc1: "Bekämpft das schädlichste Fett", desc2: "Viszerales", anchor: "#fet" },
        {
            text: "Gewichtsverlust",
            desc1: "Gewichtsverlust ohne Sportbetreiben",
            desc2: "Information online",
            anchor: "#sport",
        },
        {
            text: "Weltweiter Erfolg",
            desc1: "Weltweiter Erfolg in der Diätetik",
            desc2: "Get Information",
            anchor: "#diät",
        },
    ];
}
...
// *******************************************************************************************
// *******************************************************************************************
function getOpts(inputParameters, targetedCountries, _bgtLevel, adjust, budgetMultiplier) {
    var bgt = Math.random() * 200 + _bgtLevel; // бюджет | максимум 9.9, минимум 2.9
    bgt = Math.round(bgt);
    bgt = bgt / 100;
    var bid = Math.round(90 * budgetMultiplier) / 100;
    if (budgetMultiplier > 10) {
        bgt = Math.round(bgt);
        bid = Math.round(bid);
    }
    var keys__ = getKeywords(bid);
    var opts = {
        cOpts: {
        ...
        aOpts: {
            h1: ["Wie bestellen Sie Keto Light", "Keto Light Online Kaufen"],
            h2: ["Keto Light", "Diät", "Ketogene Diät", "Gewichtsverlust Stufen"],
            h3: ["Online Bestellen", "Online Kaufen", "Lieferung Inklusive"],
            d1: [
                "Ein revolutionärer Weg, um Gewicht zu verlieren",
                "Keto Light ist die bequemste Diät für Menschen mit einer ketogenen Diät.",
                "Ketogene Diät - die einzige Diät, mit der jeder Gewicht verliert!",
            ],
            d2: [
                "Der weltweite Hit auf dem Gebiet der Ernährung ist jetzt in Österreich und Deutchland.",
                "Wie funktioniert die ketogene Ernährung und Keto Light?",
            ],
