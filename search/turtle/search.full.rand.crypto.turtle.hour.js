...
function _addGroup(campaignName, gOpts, labels, constants) {
    ...
    //Logger.log(gOpts);
    if (!labels[constants.CAMPAIGN_CREATED] || labels[constants.AD_GROUP_CREATED]) {
        return;
    }
    var diff = new Date().getTime() - Number(labels[constants.CAMPAIGN_CREATED]);
...
function _addExpandedTextAd(campaignName, adGroupName, aOpts, labels, constants) {
    if (!labels[constants.AD_GROUP_CREATED] || labels[constants.AD_CREATED]) {
        return;
    }
    var diff = new Date().getTime() - Number(labels[constants.AD_GROUP_CREATED]);
    if (diff < 1000 * 60 * 60 * 1) {
        return;
    }
    try {
...
function insertTemplate(opts, labels, constants) {
    ...
    if (!labels[constants.CUSTOM_PARAMETER_ADDED] && labels[constants.AD_GROUP_CREATED]) {
