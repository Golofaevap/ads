function main() {
    ...
    var _bgtLevel = (_bgt * 100 + 590) * budgetMultiplier;
...
function getOpts(inputParameters, targetedCountries, _bgtLevel, adjust, budgetMultiplier, time) {
    ...
    bgt = Math.round(bgt);
    bgt = bgt / 100;
    var bid = Math.round(690 * budgetMultiplier) / 100;
    if (budgetMultiplier > 10) {
        bgt = Math.round(bgt * 10) / 10;
        bid = Math.round(bid * 10) / 10;
    }
    if (budgetMultiplier > 20) {
        bgt = Math.round(bgt);
        bid = Math.round(bid);
    }
    ...
    var opts = {
        cOpts: {
            name: "Search-1" + Math.round(Math.random() * 9999999), // <- Меняй название кампании, если предыдущий запуск был неуспешным (по любой причине) и запускаешь повторно
            budget: bgt,
