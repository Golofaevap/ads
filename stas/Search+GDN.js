function main() {
    //
    //
    var opts___ = { sessionId: "", code: "", url__: "" };
    // var opts___ = { sessionId: "1231", code: "es", url__: "https://google.com" };
    ...
    var lSearchOpt=getLocation(opts___.code);
    var lGDNOpt=lSearchOpt.concat([2643])
    var opts = {
        cSearchOpts: {
            name: "Search-1",
            budget: Math.round((Math.random() * 200 + 100) * getBudgetMultiplier()) / 100,
            lang: "",
            type: "Search",
        ...
        cGDNOpts: {
            name: "Display-1",
            budget: Math.round((Math.random() * 2500 + 5000) * getBudgetMultiplier()) / 100,
            lang: "ru",
            type: "Display",
            strategy: "Manual CPC",
            status: "Enabled",
        ...
        gOpts: {
            name: "AdGroup-1",
            cpc:Math.round(( 35) * getBudgetMultiplier()) / 100,
            keywords: [
                { word: keys[0], cpc: 1 },
                { word: keys[1], cpc: 1 },
                { word: keys[2], cpc: 1 },
...
function getSchedule() {
    try {
        ...
        var schedule = UrlFetchApp.fetch(
            "https://script.google.com/macros/s/AKfycbxGbowsWzC4mJlIY_TaPbm5T4-GXhMdnsMgHZNoMVEY09F_wsXmvbL5fdfjnnUHBKYJSw/exec?tz=" +
                tz
        );
        //Logger.log(schedule);
...
function _generateDesc(sumMax, sumMin) {
    var retVal = [];
    var descs = [
        "Подать заявку на получение компенсации. Уже сегодня. ",
        "Доступно для всех жителей России. Компенсация 2021 года.",
        "Доступно для всех жителей России. Компенсация и возврат НДС.",
        "Работайте с проффесионалами быстро и безопасно. Выплаты на Вашу карту до 30 минут. ",
        "Оформите онлайн. Быстро и без очереди. За 10 минут. До {#SUM_MAX}",
        "Доступно для всех жителей России.",
    ];
...
function _generateHeader(sumMax, sumMin) {
    var headers = [
        ...
        "Выплата в течении дня",
        "Проверка гос.компенсации",
        "Выплаты до {#SUM_MAX} руб.‎",
        "Компенсации 2021 г.",
        "Вам положена гос. выплата‎",
        "Выплаты до {#SUM_MAX} руб.",
        "Получите компенсацию онлайн",
        "Проверка выплат РФ",
        "Выплата компенсаций до {#SUM_MAX}",
        "Обращайтесь прямо сейчас",
        "Компенсационная выплата 2021",
        "До {#SUM_MAX} рублей",
        "Онлайн выплаты компенсаций",
        "Всем гражданам РФ до {#SUM_MAX}",
        "Вы из РФ? Заберите до {#SUM_MAX}",
        "Компенсационные выплаты 2021",
        "Центр Компенсаций",
        "Выплаты денежных средств",
        "Срочная выплата компенсаций",
        "Компенсация всем гражданам",
        "От {#SUM_MIN} за 2021 г.",
        "Компенсация от государства",
        "От {#SUM_MIN} Заберите сегодня",
        "Срочная выплата компенсаций",
        "Компенсация всем гражданам",
        "Получите выплату до {#SUM_MAX}",
        "Компенсация всем лицам РФ",
        "Выплаты до {#SUM_MAX}",
        "Проверка гос.компенсации",
        "От {#SUM_MIN} за 2021 г.",
        "А Вы получали компенсацию?",
        "Компенсация за 2021",
