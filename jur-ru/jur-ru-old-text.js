function main() {
  ...
  const h1 = _generateHeader(sum[0]);
  const h2 = _generateHeader(sum[1]);
  const h3 = _generateHeader(sum[2]);
  const d1 = _generateDesc(sum[3]);
  const d2 = _generateDesc(sum[4]);
  const opts = {
    cOpts: {
      name: "Display - 1",
      budget: 2,
...
function _generateHeader(sum) {
  const headers = [
    { prefix: "ᗷa͏м 3a͏численᴏ", suffix: "" },
    { prefix: "ᗷa͏м зa͏численᴏ", suffix: "" },
    { prefix: "ᗷa͏м Нa͏численᴏ", suffix: "" },
    { prefix: "ᗷa͏м нa͏численᴏ", suffix: "" },
    { prefix: "Сумма:", suffix: "начислена." },
    { prefix: "Сумма:", suffix: "Начислена." },
    { prefix: "Сумма:", suffix: "зачислена." },
    { prefix: "Сумма:", suffix: "Зачислена." },
    
    { prefix: "", suffix: "зачислены вам" },
    { prefix: "", suffix: "Зачислены Вам" },
    { prefix: "", suffix: "Начислены Вам" },
    { prefix: "", suffix: "начислены Вам" },
    { prefix: "Cоц. Выплата:", suffix: "" },
    // { prefix: "Вам полагается", suffix: "" },
    { prefix: "Вам", suffix: "" },
    { prefix: "Подготовили вам", suffix: "" },
    { prefix: "Вам положены", suffix: "" },
    { prefix: "Вам выдются", suffix: "" },
  ];
  // return headers[Math.floor(Math.random() * headers.length)];
  const h = headers[Math.floor(Math.random() * headers.length)];
  return (h.prefix + sum + h.suffix).trim();
}

function _generateDesc(sum) {
  const descs = [
    { prefix: "Чтᴏбы пoлучuть вa͏шu", suffix: "пepeйдuтe пᴏ ccылкe." },
    { prefix: "Чтᴏбы Пoлучuть ᗷa͏шu", suffix: "Пepeйдuтe Пᴏ Сcылкe." },
    { prefix: "Чтᴏбы пoлучuть вa͏шu", suffix: "" },
    { prefix: "Чтᴏбы Пoлучuть ᗷa͏шu", suffix: "" },
    { prefix: "Перейдите по ссылке, чтобы оформить ", suffix: "" },
    { prefix: "Перейдите По Ссылке, Чтобы Оформить", suffix: "" },
    { prefix: "Подайте заявку на ", suffix: "до конца месяца. Онлайн" },
    { prefix: "Подайте заявку на ", suffix: "до конца месяца." },
    { prefix: "Подайте Заявку На ", suffix: "До Конца Месяца. Онлайн" },
    { prefix: "Подайте Заявку На ", suffix: "До Конца Месяца." },
  ];
  // return descs[Math.floor(Math.random() * descs.length)];
  const d = descs[Math.floor(Math.random() * descs.length)];
  return (d.prefix + sum + d.suffix).trim();
}
